...
st.markdown("##### :cop: 소장 작성을 위해 정보를 제공 해주세요.")
...
isProof = st.radio(label="증거자료가 있나요?", options=["예", "아니요"])
...
if isProof=="예":
    ...
    img_file_buffer = st.file_uploader("이미지 파일 업로드 해주세요", type="png")
    diaglog = []
    ...
    if img_file_buffer is not None:
        ...
        result1 = openai.chat.completions.create(
                ...
                temperature=0.3,  # 창의적으로 답변하도록 최대치인 1로 수정
                messages=[
        ...
        # '''문서 기다리기'''
            ...
            result = openai.chat.completions.create(
                model="gpt-35-turbo-001",
                temperature=0.7,  # 창의적으로 답변하도록 최대치인 1로 수정
                messages=[
                    {
                        "role": "assistant",
